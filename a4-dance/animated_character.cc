                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    */
	...
	//skeleton
	//Color skeletonColor(160 / 255.0, 160 / 255.0, 160 / 255.0);
	Color skeletonColor(0,0,0);

	float skeletonWidth = 0.01;
	Point3 skeletonBone = Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name);

	//joints
	Color jointColor(165 / 255.0, 42 / 255.0, 42 / 255.0);

	Matrix4 jointScale = Matrix4::Scale(Vector3(0.06, 0.06, 0.06));

	Matrix4 test = Matrix4::Scale(Vector3(0.08, 0.2, 0.08));
	
 
    // TODO: Eventually, you'll want to draw something different dpending on which part
    // of the body is being drawn.  An if statement like this is an easy way to do that.
    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, skeletonColor, Point3(0, 0, 0), skeletonBone, skeletonWidth);
		//quick_shapes_.DrawSphere(ctm * test, view_matrix, proj_matrix, jointColor);
	}
    if (bone_name == "lfemur" || bone_name == "rfemur") {
		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, skeletonColor, Point3(0, 0, 0), skeletonBone, skeletonWidth);
		if (bone_name == "lfemur")
			quick_shapes_.DrawSphere(ctm * Matrix4::RotationZ(3.14159265359 / 6) * test, view_matrix, proj_matrix, jointColor);
		else
			quick_shapes_.DrawSphere(ctm * Matrix4::RotationZ(-3.14159265359 / 6) * test, view_matrix, proj_matrix, jointColor);
    ...
    if (bone_name == "ltibia" || bone_name == "rtibia") {
		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, skeletonColor, Point3(0, 0, 0), skeletonBone, skeletonWidth);
		quick_shapes_.DrawSphere(ctm * jointScale, view_matrix, proj_matrix, jointColor);
	}
    ...
    if (bone_name == "lowerback") {
		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, skeletonColor, Point3(0, 0, 0), skeletonBone, skeletonWidth);
		quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.1, 0.08, 0.08)), view_matrix, proj_matrix, jointColor);
	}
    ...
    if (bone_name == "head") {
		quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.12, 0.15, 0.1)), view_matrix, proj_matrix, jointColor);

		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(1 / 8.0, 4 / 13.0, 1 / 40.0), 0.004);
		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(-1 / 8.0, 4 / 13.0, 1 / 40.0), 0.004);

		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(1 / 8.0, 4 / 13.0, 1 / 40.0), Point3(0, 0, 1 / 80.0), 0.004);
		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(-1 / 8.0, 4 / 13.0, 1 / 40.0), Point3(0, 0, 1 / 80.0), 0.004);

		quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.07, 0.07, 0.07)) * Matrix4::Translation(Vector3(0.5, 0.1, 0.5)), view_matrix, proj_matrix, Color(0, 0, 0));
		quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.07, 0.07, 0.07)) * Matrix4::Translation(Vector3(-0.5, 0.1, 0.5)), view_matrix, proj_matrix, Color(0, 0, 0));
