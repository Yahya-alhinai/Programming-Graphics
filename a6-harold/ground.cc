                           ...
                           const std::vector<Point2> &stroke2d)
{
	...
	Vector3 planeNormal = (start - end).Cross(Vector3(0, 1, 0));
	planeNormal = Vector3::Normalize(planeNormal);

	if (planeNormal.Dot(look) > 0) planeNormal = planeNormal * -1;
	
	// 2. Project the 2D stroke into 3D so that it lies on the "projection plane"
    // defined in step 1.
    
    
	std::vector<Point3> silhouette;
	for (auto& pt : stroke2d) {
		Point3 mouseIn3d = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, pt);
		Ray eyeThroughMouse(eye, (mouseIn3d - eye).ToUnit());

		float t;
		Point3 curve;
		eyeThroughMouse.IntersectPlane(start, planeNormal, &t, &curve);
		silhouette.push_back(curve);
	}

	for (int i = 0; i < stroke2d.size(); i++) {
		Point2 pt(stroke2d[i].x(), stroke2d[i].y());
		Point3 mouseIn3d = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, pt);
		Ray eyeThroughMouse(eye, (mouseIn3d - eye).ToUnit());
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
		...
		float h = hfunc(planeNormal, silhouette, P.ClosestPointOnPlane(start, planeNormal));
		...
		if (h)
		{
			...
			P = Point3(P.x(), (1 - wd) * P.y() + wd * h, P.z());
		}
